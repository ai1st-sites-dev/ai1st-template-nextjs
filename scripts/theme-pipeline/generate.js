...
// 这个文件（成环）。两处各算一遍同一件事就会分叉，这条流水线为此付过账（#1051 的 `heroLayoutFor`）。
const { paletteFor } = require('./palette.js');

// 🔴 契约版本从 `theme-css-lint.js` 自己那里取，**不手抄**（#1171，来源 #1162）。同族另外两处已经
// 这么做了：`sheet-fresh.js` §check（它判的是「被判的那棵树」自己的版本）和 `gates.js`
// §lintCanSeeLiteralColours（那一处还写着手抄的后果：探针在**版本行**上就被拒掉，于是那个函数把
// 「拦不拦得住字面色值」读成通过 —— 方向反了的假通过）。这里手抄的后果是另一头，也一样静默：
// 升版本那天生成器还在往表头写旧版本号，而 lint 会把它产出的**每一份**表都拒掉，报的却是版本行。
// require 是安全的：那个文件用 `require.main === module` 把命令行入口关在外面。
const { CONTRACT_VERSION } = require('../theme-css-lint.js');
...
// 📌 #1341 之前版式那个名字也从这里出（`<id>.layout.json` 写的就是它）；那一维退役了。
const { sheetFor, voiceFor } = require('./sheet-recipes.js');
...
function generateCandidates(n = 3, { seed = 7, outDir } = {}) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    const sheet = `/* theme-css-contract: ${CONTRACT_VERSION}\n   ${id} — generated by scripts/theme-pipeline/generate.js`
      + ` (#1004; all 34 blocks since #1051). */\n\n${sheetFor(i, seed)}`;
    // 📌 #1341 —— 这里原来还产一个 `layout` 键（`layoutNamesFor(i)` 的四个版式名），`promote.js`
    //    再把它的每一个键翻成 `theme-pool.json` 的 `supports` 清单。内容结构那一整维退役了，
    //    所以这里不产它、`<id>.layout.json` 也不再写；池成员的 `supports` 只剩顶栏 / 页脚，由
    //    `promote.js` 的 `regionsForPool` 算。
    // 🔴 代价写在明处：相似度闸（`gates.js` §similarity）的 `layout` 分项因此**恒是 null**、被加权
    //    平均从分母里去掉 —— 那道闸照旧出分，只是少了一维证据。闸的输出会把这件事说出来
    //    （`partsText` 打 `layout 没法比（不参与评分）`），不是静默。
    const entry = {
      id,
      tokens,
      // #1342 —— 第六道闸（`gates.js` 的 `gateShapes`）问的就是这个键：`blocks/` 里每个块一个形态名，
      // 不多不少。名字从块 manifest 的 `shapes` 清单里挑，分母和取值范围都现读，见 shape-sheet.js。
      // 🔴 #1341 —— 这一行上面原来还有一个 `layout: layoutNamesFor(i)`（内容结构那一维），
      //    随本票整族退役了。合并这两票时**只能丢 layout、留 shapes**：机械取任一侧都产出坏树
      //    （取本票这一侧 ⟹ #1342 的 `shapes` 被静默回退；取 main 那一侧 ⟹ 当场
      //    `ReferenceError: layoutNamesFor is not defined`，那个函数已随本票退役）。两个方向 QA1/QA2
      //    各量到一个，写在这儿免得下一个合并的人再踩。
    ...
    if (outDir) {
      ...
      // #1342 —— 选择单**不进** tokens 文件（tokens 有自己的 schema，多一个键就该被第一道闸点名），
      // 也不能只活在内存里（`run.js --candidates` 那条路从磁盘读回候选，读不回来的话第六道闸对它
      // 恒拒）。文件名只算一处（`shapeSheetPath`）—— 写它的是这里，读它的是另一个进程里的
      // `promote.js`。
      // 🔴 #1341 —— 这上面原来还有一次写盘（`<id>.layout.json`，内容结构那一维），随本票退役；
      //    合并两票时只丢它、不碰下面这一行，理由整段在上面 `shapes:` 那里。
...
if (require.main === module) {
  ...
  // 📌 #1341 —— 这一行原来第二列印的是 `c.layout.hero`（那套候选的内容结构）。那一维退役了，
  //    改印画法名 —— 它是这个文件今天真的在挑的那一维（`voiceFor(i).heroLook`）。
  console.log(cands.map((c, i) => `${c.id}  ${voiceFor(i).heroLook}  primary-500 ${c.tokens.colors.primary['500']}`).join('\n'));
}
