...
// ── AC1 存量不误伤：80 套逐套当候选，跟其余 79 套跑一遍真的闸 ─────────────────────────────────────
{
  const rejected = [];
  const jammedOn = [];
  for (const id of IDS) {
  ...
  else ok(`AC1 80 套逐套跟其余 79 套跑真闸 ⟹ 被拒集合为空、没有一套量不到`);
...
{
  const FROZEN_POOL = 80;
  // sha256(排序后的池 id 逗号连接).slice(0,16) —— 只认「是不是同一批成员」，跟任何读数无关。
  const FROZEN_POOL_ID = '573e6144cb2ce2da';
  ...
  const FROZEN_HIST = {
    3: 5, 4: 45, 5: 143, 6: 203, 7: 227, 8: 103, 9: 2434,
  };
  const FROZEN_CLOSE = [
  ...
  if (stale) {
    ...
    if (distinctSame && histSame && closeSame) {
      ok('AC2 每块 distinct、3160 对的距离分布、≤3 那 5 对、每对不同的是哪几块 —— 与正文那把尺逐个相同');
    } else {
