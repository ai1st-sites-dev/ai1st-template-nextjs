...
// （`themes.js` 的 `shapesFor`，`sync-config.js` 的 `shapeForBlock` 读它）。这道闸立起来之前，选择单
// 不齐的候选**直接进池**：`promote.js` 的 `toPoolEntry` 当时根本不写 `shapes` 这个键，而进池之后才有
// `pool.test.js` 第 ⑪ 段去数它 —— 也就是要等写完池、下一次跑测试时才红，而那时坏的已经是池子本身。
// 这道闸把同一句话提前到进池之前说，并且点名缺了谁 / 多了谁。
//
// 🔴 **这道闸是可过的**（#1342）。立起来那天生成器一套选择单都不产 ⟹ 每套候选都在这里被拒，而那正是
//    当时要的方向：「函数在、接线全无」的那种半哑状态（闸量过了、池子里却是一套没有选择单的主题）
//    比当场红贵得多。#1342 补的是被拒的那一端 —— `generate.js` 现在按 `shape-sheet.js` 的
//    `shapeSheetFor(i, seed)` 给每个块挑一个**在该块 manifest 清单里**的名字、落成 `<id>.shapes.json`，
//    `promote.js` 再把它搬到池成员的 `shapes` 上。**当时那句「要让流水线重新走通，是让候选带上选择单，
//    不是把这道闸放宽」一个字没改，走的就是它那条路：这道闸的判据一个字节没动。**
//
