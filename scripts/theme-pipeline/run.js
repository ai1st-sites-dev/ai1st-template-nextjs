...
async function main() {
  ...
  //    也是那个、第③道闸报告里的 id 也是那批。喂满整池能绕开，代价是「池子有几套就跑几次真构建」
  //    （#1174 那天是 97 次；#1317 之后池子是 2 套，而位子表仍然是 97 个位子 —— 绕开那条路的代价
  //    按**位子数**算，不是按今天池子里有几套算）。
  ...
  //    📌 说在明处：**这个文件里 2 有一大把占用方** —— 现取 `grep -cE 'process\.exit\(2\)'` = 10（别抄这个数，自己取）
