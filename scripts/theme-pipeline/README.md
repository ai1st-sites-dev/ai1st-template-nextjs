...
| ⑥ 选择单 | 这套候选给 `blocks/` 里**每个块一个画法名，不多不少**（#1338）。读的是候选自己的 `shapes`，不用建站，所以排在 ② 之前。🔴 **这道闸是可过的**（#1342）：立起来那天生成器一套选择单都不产 ⟹ 候选被全部拒掉；#1342 补的是被拒的那一端 —— `generate.js` 按 `shape-sheet.js` 的 `shapeSheetFor(i, seed)` 给每个块挑一个在该块 manifest 清单里的名字、落成 `<id>.shapes.json`，`promote.js` 搬到池成员的 `shapes` 上。**闸的判据一个字节没动** | `gates.js` |
...
🔴 **编号不是执行顺序：⑤ 跑在 ④ 之前**（骨架双胞胎不该浪费人审），**⑥ 跑在 ② 之前**。给它们新号而不是
插成新的 ④ / ②，是因为票、报告、memory 里已经有几十处按名字引用那几道，重编号会让那些引用全部指错。

🔴 **⑥ 不问「名字合不合法」**，只问「键对不对得上 `blocks/` 的名单」。名字那一维靠
`pool.test.js` 第 ⑪ 段（名字在 `public/shapes.css` 里有规则）加 `scripts/lib/block-shapes.test.js`
第 ① 段（manifest 的形态清单 == `shapes.css` 的集合）接起来，而那两道跑在**池成员**上、不跑在候选上
—— 候选阶段名字写错，要等它进了池才有人说话。
