...
| ⑥ 选择单 | 这套候选给 `blocks/` 里**每个块一个画法名，不多不少**（#1338）。读的是候选自己的 `shapes`，不用建站，所以排在 ② 之前。🔴 **今天的生成器一套选择单都不产**（`promote.js` 的 `toPoolEntry` 不写 `shapes`）⟹ 生成出来的候选会在这里被全部拒掉。那是要的方向：让候选带上选择单，不是把这道闸放宽 | `gates.js` |
