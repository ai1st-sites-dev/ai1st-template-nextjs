...
// 共新增 17 个位子，位子表 80 → 97。
//
// 🔴 **位子表 ≠ 今天池子里有几套主题。** #1317（2026-09-14）把 `theme-pool.json` 从 97 套删到 2 套
// （整池按新框架重建），而这张位子表**一个字没动** —— 动它会让后面每一组的位子序号全体后移，而位子
// 序号是主题 id（`promote.js:86-87`）和 hero 画法的输入，已建站下次 rebuild 第一屏就换样。所以
// 重建期「位子 97 · 池子 2」是有意的状态，盯这两个数相等的那几格测试按池子大小门控了
// （`scripts/lib/scaffolding-pool.js`），池子重生成到 ≥10 套那天它们自己回来。
//
