...
Available section types: hero, hero-with-form, trusted-brands, features-grid, card-group, testimonials, cta-banner, contact-info, text-block, page-header, services-nav, services-list, quote-form, contact-form, stats-counter, faq-accordion, process-steps, team-grid, pricing-table, gallery, content-split, feature-comparison, social-proof, divider, announcement-bar, timeline, newsletter-signup, map-area, awards-certifications, blog-preview, service-related-pages
...
async function main() {
  ...
  let archiveError = '';
  // 🔴 下面 MODEL_PRICING 不是文档,是【记账输入】:getModelPricing(model) 的结果乘 token 数既写进 operation_runs.cost,又经 manager/credit_metering.go 的 debitEditCredit 真扣客人额度。写错一行不报错、只静默错记。
  const model = configModel;
  // 🔴 改它之前去 https://platform.claude.com/docs/en/about-claude/pricing 现取一次,别凭记忆 —— #1249 修的两行原来逐字是【已退役】型号的真价钱(Opus 4/4.1 = $15/$75 · Haiku 3.5 = $0.80/$4),不是打错,所以判据是逐行拿官方现价对一遍,不是「哪个数看着不对」。
  // $/M tokens by model family。🔴 查表是 startsWith 前缀匹配,两处盲区:① 'claude-opus-4' 同时罩 opus 4/4.1($15/$75,已在一方 API 退役 ⟹ 调不通、不会产生 cost 行)与 4.5~4.8($5/$25),按【可达的那一种】取值;② 不以这三个前缀开头的 id(claude-opus-5 / claude-sonnet-5 / claude-fable-5 …)静默落到下面的 return {input:3,output:15} = 按 sonnet 4.6 记账 —— sonnet-5 官方 $2/$10 ⟹ 那个方向是【多记 50%】,多扣客人钱。
  const MODEL_PRICING = {
