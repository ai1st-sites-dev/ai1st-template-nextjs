 ...
 * 🔴 #1161 —— 这两句以前写的是「注册表 **110 套**里有 **10 套**过不了（最低的 `golden-yellow` 白字压
 * primary-500 = 1.92:1；10 套**全部**落在退役那 30 套里，在池的 80 套一套都没有）」。那 30 套
 * 2026-08-23 整体下架之后，注册表就是那 80 套 —— 也就是说**按老写法的白字判据，今天注册表里
 * 一套都不过不了**（那 10 套全在下架的那批里）。这不是「问题解决了」，是量点换了；这一段的结论
 * （六组预设是量过挑出来的，不是抄哪套都行）不依赖那个数。数一律现算，别抄这里 ——
 * 下面这六组是量过之后挑出来的，判据在 `scripts/theme-presets.test.js` 的第 ① 节（它同时把那个判据
 * 打在整个注册表上，**再加上 `.btn-accent:hover` 那一对 gray-900 压 accent-500，报红 11 套**，
 * 证明它分得开好坏、不是恒绿）。
 *
 ...
 *    §.btn-accent:hover，#1100 起底色是算出来的、兜底 `--color-accent-500`）⟹ **11 套**；
 *    误取成 `白字 / accent-500` ⟹ **109 套**。同一个注册表、同一把尺，两个数差 98 套。
 *
 ...
 * 📌 `scripts/theme-css.js` 的 `--font-sans` / `--font-heading` 那两行今天是直接 `join(', ')` 的，
 * 而它能用**不是因为字体名里没有数字**。2026-08-21（origin/main 7be6d585） 实测：注册表 110 套里有 **14 套**的正文字体
 * 就是 `Source Sans 3`（带数字）。真正的原因是**注册表存的家族名本来就是带引号的**——
 * `indigo-03.fonts.body` 逐字是 `['"Source Sans 3"', 'system-ui', 'sans-serif']`，引号在数据里，
 * 所以 `join` 出来的串已经合法。
 * 🔴 #1134 更正的就是这一句（原文：「能用只是因为 30 套主题的字体名恰好一个数字都没有」）——
 * 它三处都不对：分母不是 30、数字确实存在、而安全性来自另一件事。**结论仍然成立**：那不是一条保证，
 * 只是当前这批数据的样子——一条**没有加引号**的多词家族名进了注册表，那两行就会产出无效 CSS，
 * 而上面 `cssFamily()` 正是给这件事做的（写入路径过它，`theme-css.js` 那两行不过它）。
 */
