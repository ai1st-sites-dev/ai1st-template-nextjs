...
for (const locale of locales) {
  ...
  {
    const cov = blockManifest.registryCoverage(path.join(rootDir, 'src', 'lib', 'sections', 'registry.generated.ts'));
...
// `:root` 默认值上。**两者不是同一组数**：**#961 那 30 套手写主题**里只有 3 套的设定恰好等于默认值，
// 其余 27 套不是（本轮现读：`sharp/standard/square` 3 套、`round/airy/pill` 5 套、
// `sharp/compact/square` 5 套…）。🔴 语料写在这里（#1140，来源 #1083）：那 30 套是 #1161 下架的
// 那一批。🔴 **别拿「在 `retiredThemes` 里」当它们的判据** —— #1317（2026-09-14）又把池子里 95 套
// 下架搬了进去，那份名单 30 → 125。上面那几个数**只对最早那 30 套成立**
// —— 池主题的 settings 是**数值形状**（`radius: 16`），根本不走档位表。只读 globals.css
