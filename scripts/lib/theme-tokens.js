...
//   · 枚举形状（#961 那 30 套手写主题）：radius: 'sharp' · …
//     🔴 #1161（2026-08-23）之后这两组不再住在同一个导出里：`themes` 就是池子，枚举形状的那 30 套
//     在并列的 `retiredThemes` 里。
//     🔴 #1317（2026-09-14）起**不能再拿「在 `retiredThemes` 里」当「是枚举形状」的判据**：那份
//     名单 30 → 125，新进来的 95 条是从池子下架的、数值形状的主题。要分形状就读 `settings.radius`
//     的类型（字符串 = 枚举，数字 = 数值）。
