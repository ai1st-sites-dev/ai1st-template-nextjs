...
//
// 🔴 #1376 按文件头那条维护约定把它从 `9d056fd7`（#1346 的 base）往前挪到了 `4b0e53b4`（#1353 落地
//    那个 commit），理由是「OFF 那条路的字节确实该变」：#1376 按设计文档 D19 从区块库里删掉一个块，
//    提示词里那句「There are N section types」于是从 32 变成 31 —— 而 `9d056fd7` 那份 create-site.js
//    把这个数**写死成 32**（#1353 之后才改成按 `blocks/` 现算）。两臂共用这棵树的 `blocks/`，所以钉在
//    写死那一版上，块库一少就必然对不上，且差的正是那个数字。实测（本票交付那份字节上现取）：
//      基线 `9d056fd7` vs 工作树 —— 两份都是 17568 字节，diff **只有第 221 行**那句 32 / 31
//      基线 `4b0e53b4` vs 工作树 —— **逐字节相同**（两版都现算，读的是同一份 `blocks/`）
//    挪过去之后这一格照旧只对 scripts 的行为说话。
const BASELINE = '4b0e53b4';
