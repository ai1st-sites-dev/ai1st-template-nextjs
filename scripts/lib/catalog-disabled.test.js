...
//
// 🔴 #1375 按文件头那条维护约定把它从 `9d056fd7`（#1346 的 base）往前挪到了 `4b0e53b4`（#1375 的
//    base），理由是「OFF 那条路的字节确实该变」：#1375 按 D19 从区块库里删掉一个块，提示词里那句
//    「There are N section types」于是从 32 变成 31 —— 而 `9d056fd7` 那份 create-site.js 把这个数
//    **写死成 32**（#1353 之后才改成按 `blocks/` 现算）。两臂共用这棵树的 `blocks/`，所以钉在写死
//    那一版上，块库一少就必然对不上，且差的正是那个数字。挪到 `4b0e53b4` 之后两臂都现算、都读到
//    同一份 `blocks/`，这一格照旧只对 scripts 的行为说话（实测：改动前两臂逐字节相同 17780，
//    改动后 17590 / 17590 只差那一行）。
