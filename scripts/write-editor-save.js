...
//
// 成功时 stdout 打**一行** JSON：{"ok":true,"files":["site/en/pages/home.json","site/theme.json",…]}
...
try {
  ...
  const writes = [];
  if (hasPage) {
    writes.push(pageWrite.planPageWrite({ root: ROOT, blocks, pageFiles, target, slug, baseHash, next: input.page }));
  }
