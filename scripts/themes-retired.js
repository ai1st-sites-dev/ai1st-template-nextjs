// ══════════════════════════════════════════════════════════════════════════════════════════════════
// themes-retired.js — 冻结退役的旧 30 套（#1016，spec D3）
// ══════════════════════════════════════════════════════════════════════════════════════════════════
//
// 这 30 套是 #924 到 #961 之间手写的那一池。阶段 3 用新池整个换掉它们（spec §8「AI 生成新池，旧 30 套
// 冻结退役」），退役的意思**只有一条**：新建的网站不再抽到它们 —— `candidateThemesForIndustry()`
// 只在新池里挑。
//
// 🔴 它们**没有被删掉**，而且必须删不得：
//   · 今天线上每一个站的 `site/theme.json` 写的都是这 30 个里的某一个 id。`sync-config.js` 对
//     `applied: true` 的站会 `themes[meta.themeId]` 查注册表，查不到就 `process.exit(1)` —— 也就是
//     那个站从此**建不出来**。删掉 = 把老站砖掉。
//   · `themeStyle()` 拿它喂 logo 提示词，`layoutFor()` / `settingsFor()` 是构建期读的。
//   ⟹ 所以 `themes.js` 导出的 `themes` 仍然是「新池 + 这 30 套」的并集（查得到），而**挑选**那条路
//     只走新池。两件事在 `themes.js` 底部分得很清楚。
//
// 🔴 内容逐字节没动，只是从 `themes.js` 搬到这里（判据：搬前搬后 `JSON.stringify` 逐字相同，
//    读数在 #1016 的交接留言里）。要改这里的任何一套之前先想清楚：改的是**已经上线的站**的样子。
const retiredThemes = {
  'bold-red': {
