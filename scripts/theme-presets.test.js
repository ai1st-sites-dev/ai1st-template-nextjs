...
let judgeSheetForRegistrySweep = null;
{
  ...
  //   几何一次都没被读。今天同样如此 —— 只是它现在**新鲜**,而不是「过期但用不上」。
  {
    ...
    if (checked === 0) {
      ...
      // 🔴 **这一支今天在干净 main 上就响,所以它【是】背景噪音**(#1238 打磨批次 #27 条 9,来源 #1234)。
      //    历史:#1096 / #1134 那两轮它 3/3 恒响;#1149 用真浏览器重取了那三份几何,当天干净 main 上
      //    读到 0/3,于是这里曾经写着「你现在看到它响就是有东西真的变了」。**那句话已经过期了** ——
      //    2026-08-29 在干净的 `origin/main` `6c692347` 上现测(`node --test scripts/theme-presets.test.js`,
      //    `public/themes/` 与 `theme-text-bands.json` 一个字节都没碰)读到的是 **3/3 已过期**,
      //    点名的还是同样那三份:hero-media-left · hero-media-right · hero-media-top。
      //    🔴 失败方向是坏的那个:那句话会教下一个人把 3/3 读成「**你**改坏了东西」。
      //    ⟹ 别在这里再钉一个「干净 main 上是几比几」的数(它每被重生一次就翻一次面)。要判断
      //    「是不是我改的」,自己在干净树上跑一次同一条命令,拿两个读数比。
      ...
      warn(`theme-text-bands.json 里 ${stale.length}/${checked} 份几何读数**已经过期**:${stale.join(' · ')}`
        ...
        + ' 🔴 **它今天在干净 main 上也响** —— 2026-08-29(#1238)在 origin/main 6c692347 上现测就是 3/3,'
        + ' 点名的是同样那三份。所以【别】把它读成「你刚刚改坏了什么」:要判断是不是你,'
        + ' 在干净树上跑一次同一条命令,拿两个读数比。'
...
// `hero-media-*`，而一个站穿哪张表由它自己的主题名决定（`create-site.js §main` → `theme-sheet.js:41`），
...
//       这一圈**今天没有任何东西在判**（📌 下面举的头一个钩子 #1372 随块删掉了，读数原样留着）：
//       里面大量是刻意压低的装饰件（一个对比块的 `--no` 修饰钩子
...
// 配色上量的，报告里的是当天现算的。
{
  ...
  // 自己的主题名决定（`create-site.js §main` → `theme-sheet.js:41`）。所以「80 份表 × 110 套配色」
