...
let judgeSheetForRegistrySweep = null;
{
  ...
  //
  // 🔴 为什么这里是 ⚠️ 而不是 ❌:三张表存着的 md5 **全部**已经对不上,而且这是个**长期状态**,
  // 不是某一天的事故。两次独立复量(存量那一栏一直没变,变的是「实际」):
  //     2026-08-19 (#1096, f110380e 与 a382db2b 两次读数相同)  实际 b0e49963 / 1a34e960 / c41b73ff
  //     2026-08-21 (#1134)                                     实际 b90ee644 / eb346671 / 139ffdeb
  //     存量(两次都是)                                          83b9bd9b / 7f6ae9bd / b35f4919
  // ⟹ 表在两次之间又改过。**这几个 md5 一个都不许被当成参照**,它们是那一天现算出来的。直接
  ...
  // 摆给作者定夺「重取 + 升成 ❌」。
  {
    ...
    if (checked === 0) {
      ...
      // 🔴 #1134（来源 #1096）—— 这句话必须先说清「它是一个**长期状态**,不是一次新事件」。
      //    #1096 自己的注释写着「一个在正常 ship 上就响的门,人会立刻学会忽略它」,而这一句
      //    **今天在干净 main 上必然响**(#1096 交付时 3/3 过期,#1134 复量仍是 3/3)。于是
      //    「真的刚被弄坏」和「今天本来就这样」在屏幕上长得一模一样。
      // 🔴 而且「实际」那一栏自己也在动 —— 同一份存量几何,两次复量读到的是不同的实际 md5:
      //      #1096 量到  b0e49963 / 1a34e960 / c41b73ff
      //      #1134 量到  b90ee644 / eb346671 / 139ffdeb
      //    表在这两次之间又改过。⟹ **别拿任何一次写下来的 md5 当参照**,那是这一格自己现算的。
      warn(`theme-text-bands.json 里 ${stale.length}/${checked} 份几何读数**已经过期**:${stale.join(' · ')}`
        + ' —— 那张表改过了,存着的那段几何是关于旧字节的。哪天这张表破线,判它的就是一份过期几何。'
        + ` 🔴 先读这一句再决定要不要查:${stale.length}/${checked} 过期在今天的 main 上是**已知的长期`
        + '状态**(#1096 交付时如此,#1134 复量仍如此),它不说明本次改动弄坏了什么。'
        + ` **能说明问题的读数是它变小**(有人重取过)**或者变大到 ${checked}/${checked} 之外**。`
        + ' 要让它闭嘴只有一条路:拿真浏览器重取一次 ——'
