...
| `variant-screen.js` | **搬之前**的普查（#1031 用它出批 F 的判断表）：对着组件源码逐块数出「有几支 `variant ===` 分支、每支读了哪些 `data.*` 字段」，再对着线上语料数出「每个变体值今天有几个实例」。判断表要回答的那一句 —— 这个差别是**内容结构**（归 `block_layout`，站说了算）还是**只是长相**（归主题表）—— 判据是**逐支的字段集**，不是变体的名字（`page-header` 的 `with-description` 听着像内容结构，三支量完字段完全相同）。**它是普查工具，不是断言工具**：没有退出码语义，读数要人看。 |
