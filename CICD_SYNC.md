...
| `ai1st-sites/ai1st-template-nextjs` | prod | **Manual promote** — run the `Deploy template → prod` workflow (`deploy-template-prod.yml`). It copies the dev mirror's current bytes to prod, so prod always gets exactly what was validated on dev. |
