...
import { brand, navigation, services, allPages } from '@/lib/config';
            ...
            <div key={column.title}>
              ...
              <ul className="space-y-2">
                {column.links.map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} className="text-sm transition-colors hover:text-white">{link.label}</Link>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {services.slice(0, 6).map((service) => (
                <li key={service.id}>
                  <Link href={serviceDetailSlugs.has(service.id) ? `/services/${service.id}` : `/services#${service.id}`} className="text-sm transition-colors hover:text-white">{service.name}</Link>
                </li>
              ))}
            </ul>
          </div>
