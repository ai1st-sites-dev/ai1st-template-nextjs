...
// announcement-bar · divider · newsletter-signup · trusted-brands · blog-preview · social-proof，
// 外加两个已经不在注册表里的：批次 E 那个 logo 墙块（#1375 按 D19 删掉）和那个显示一排数字的块
// （#1376 按 D19 并进 `social-proof`），所以这里不再写它们的名字；那次读数本身没变，零交集这个
// 结论也没变）。要是有交集，就会出现「预览里挡住了不许藏，Apply 之后 sync-config 又真的把它藏了」
// ——两个地方对同一件事给出相反答案，而那正是本票要治的那类假象。
//
